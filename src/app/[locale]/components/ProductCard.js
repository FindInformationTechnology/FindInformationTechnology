...
import WatchProductImage from "../../../../public/products/apple-watch.png";
...
const ProductCard = () => {
  ...
  const handleRedirection = (param) => {
    if (isMounted) {
      ...
      window.location.href = `${pathname.slice(0, 3)}/products_details?id=${
        param?.id
  ...
  return (
    <Card
      className="offers-card cursor-pointer"
      onClick={() =>
        handleRedirection({ id: 0, name: "iphone 14 pro", price: "5500AED" })
      }
    >
      <Card.Body className="card-wrapper">
        <div className="product-image-wrapper">
          ...
          />
          <Button
            className="add-to-favorites-btn z-10"
            onClick={(e) => {
              e.stopPropagation();
