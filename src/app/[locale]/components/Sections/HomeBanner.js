...
export default function HomeBanner() {
    ...
    const suggestionsList = () => {
      if(pathname.slice(0,3)==='/ar') {
        return(suggestionsAR.map((item, index) => (
          <div key={index} className="suggestion-item min-w-fit">
            {item}
          </div>
        )))
      } else {
  ...
  return (
      <Container className="search-section min-w-full  mb-5 py-5">
        <h2 className="font-light text-[24px] md:text-[48px] min-w-fit my-[6.5vw]">{t('title')}</h2>
        <div className="search-container mx-auto container m-0 p-0 gap-2 mb-5">

            <div className="flex items-center w-full search-input-wrapper m-0 p-0">
            <Form.Control
                type="text"
                placeholder={`${t('inputPlaceholder')}`}
                value={searchTerm}
                onChange={handleSearchChange}
                className="search-input h-[56px] p-3 m-0"
            />
        ...
        <div className="search-suggestions m-0 p-0">
          {suggestionsList()}
        </div>
      </Container>
  );
