...
export default function Products() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      setParams(query.get("key") || "No name provided");
      // console.log(params);
    }
  }, []);
  ...
  return (
    ...
    <Container className="m-0 min-w-full max-w-full bg-[#FAFAFB]">
      ...
      <Row className="justify-between">
        ...
        <Col xs={12} sm={9} md={9} lg={9} className="m-0 p-0 mt-4">
          {params === "offers" && (
          ...
          <div>
            <div className="col-header hidden md:flex flex-col md:flex-row items-center justify-between w-full">
              ...
              <div className="flex items-center gap-1">
                <span className="text-[14px] font-semibold">{t("sortBy")}</span>
                <div className="flex items-center gap-1">
                  <span className="text-[14px]">{t("MostPopular")}</span>
                  <IoMdArrowDropdown size={14} />
                </div>
              </div>
            </div>

            <Row className="products-cards-show-row min-w-full max-w-full mt-4 gap-x-0  gap-y-3">
              {products &&
                products.map((item, index) => (
                  <Col md={4} key={index} className="m-0 px-1">
                    <ProductCard />
              ...
              <div className="flex items-center justify-center mb-[5rem]">
                <div className="primary-btn-outline mx-auto mt-4 mb-[10rem]">
                  {t("ViewMore")}
                </div>
              </div>
            </Row>
